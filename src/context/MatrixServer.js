...
const MatrixServerProvider = ({ children }) => {
  ...
  const updateContacts = (server) => {
    ...
    const reqRoomsInfo = rooms
      .map((room) => {
        try {
          const members = room.currentState._displayNameToUserIds;
          const otherMember = Object.keys(members).filter(
            (key) => !members[key][0].startsWith("@" + username)
          )[0];
          return {
  ...
  return (
    <MatrixServerContext.Provider
      value={{ setServerUrl, server, updateContacts }}
    >
      {children}
    </MatrixServerContext.Provider>
  );
